<x-layouts.app title="لیست قرضدار ها">
    <x-ui.legacy-list title="لیست قرضدار ها" report-only table-id="creditors-grid">
        <x-slot:subtabs>
            @include('reports._nav')
        </x-slot:subtabs>

        <table class="legacy-grid" id="creditors-grid">
            <thead>
            ...
            <tbody>
                @foreach($rows as $row)
                    <tr data-row>
                        ...
                        <td class="{{ $row['balance'] >= 0 ? 'text-rose-700' : 'text-emerald-700' }}" style="font-weight:700">{{ number_format($row['balance'], 2) }}</td>
