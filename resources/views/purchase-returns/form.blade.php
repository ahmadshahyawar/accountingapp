<x-layouts.app title="برگشت از خرید">
    <x-ui.page-header title="برگشت از خرید جدید" :back-route="route('purchase-returns.index')" />
    ...
    <form action="{{ route('purchase-returns.store') }}" method="POST"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6">
            ...
            <x-ui.field label="تامین‌کننده" name="person_id" type="select" required :options="['' => '— انتخاب —'] + $suppliers->pluck('name', 'id')->all()" />
        ...
        <button type="submit" class="px-6 py-2 bg-amber-700 text-white rounded-md text-sm hover:bg-amber-800">ثبت برگشت از خرید</button>
    </form>
</x-layouts.app>
