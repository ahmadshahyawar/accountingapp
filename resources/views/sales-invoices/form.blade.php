<x-layouts.app title="فاکتور فروش">
    <form action="{{ route('sales-invoices.store') }}" method="POST"
        x-data="{
            items: @js($items->map(fn($i) => ['id' => $i->id, 'code' => $i->code, 'name' => $i->name, 'price' => (float) $i->sale_price, 'cost' => (float) $i->cost_price, 'unit' => $i->unit->name])),
            customers: @js($customers->map(fn($c) => ['id' => $c->id, 'name' => $c->name, 'phone' => $c->phone, 'mobile' => $c->mobile])),
            proformas: @js($proformas->map(fn($p) => ['id' => $p->id, 'label' => $p->number.' — '.$p->customer_name, 'warehouse_id' => $p->warehouse_id, 'currency_id' => $p->currency_id, 'fx_rate' => (float) $p->fx_rate, 'notes' => $p->notes, 'lines' => $p->lines->map(fn($l) => ['item_id' => $l->item_id, 'quantity' => (float) $l->quantity, 'unit_price' => (float) $l->unit_price])])),
            ...
            addLine() {
                ...
                this.lines.push({ item_id: this.pending.item_id, code: it.code, name: it.name, unit: it.unit, quantity: this.pending.quantity, unit_price: this.pending.unit_price, cost: it.cost, discount: 0 });
                this.pending = { item_id: '', quantity: 0, unit_price: 0 };
            },
            removeLine(i) { this.lines.splice(i, 1) },
            lineTotal(l) { return (l.quantity * l.unit_price) - (l.discount || 0) },
            lineProfit(l) { return this.lineTotal(l) - (l.cost * l.quantity) },
            ...
            loadProforma(id) {
                ...
                this.lines = p.lines.map(l => {
                    ...
                    return it ? { item_id: it.id, code: it.code, name: it.name, unit: it.unit, quantity: l.quantity, unit_price: l.unit_price, cost: it.cost, discount: 0 } : null;
                }).filter(Boolean);
                this.showProformaPicker = false;
            },
        }">
        @csrf
        ...
        <div class="grid-toolbar" style="border-top:none;border-bottom:1px solid #d7dce1;padding:8px 2px 12px;margin-bottom:0">
            ...
            <div class="legacy-form-title" style="flex:2;padding:0">فاکتور فروش</div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 my-4 items-start">
            ...
            <div class="legacy-field">
                <label>انبار<span class="text-red-600">*</span></label>
                <select name="warehouse_id" x-ref="warehouse" required data-searchable>
            ...
            <div>
                <div class="legacy-field" style="margin-bottom:8px">
                    ...
                    <select name="currency_id" x-ref="currency" required data-searchable>
                ...
                <div class="legacy-field" style="margin-bottom:0">
                    ...
                    <input type="number" step="0.0001" name="fx_rate" x-ref="fxRate" value="{{ old('fx_rate', 1) }}" required>
                </div>
            </div>
        </div>
            ...
            <tbody>
                <template x-for="(line, index) in lines" :key="index">
                    <tr>
                        ...
                        <td x-text="lineProfit(line).toFixed(2)"></td>
                        <td>
                ...
                <tr x-show="lines.length === 0"><td colspan="8" style="text-align:center;color:#9aa3ab;padding:20px">هیچ جنسی اضافه نشده است</td></tr>
            </tbody>
        </table>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            ...
            <div class="order-1 md:order-3 legacy-field" style="margin-bottom:0">
                ...
                <textarea name="notes" x-ref="notes" rows="6">{{ old('notes') }}</textarea>
            </div>
        </div>
        ...
        <div class="grid-toolbar">
            ...
            <div class="grp" style="position:relative">
                @if($proformas->isNotEmpty())
                    <button type="button" class="btn3d" @click="showProformaPicker = !showProformaPicker">
                    ...
                    <div x-show="showProformaPicker" x-cloak style="position:absolute;bottom:100%;left:0;background:#fff;border:1px solid #7fb3e0;border-radius:6px;box-shadow:0 8px 22px rgba(20,40,60,.2);width:260px;margin-bottom:6px;z-index:20">
                        <template x-for="p in proformas" :key="p.id">
                            <button type="button" @click="loadProforma(p.id)" x-text="p.label" style="display:block;width:100%;text-align:right;padding:8px 10px;font-size:13px;border:none;background:none;cursor:pointer"></button>
                        </template>
                    </div>
                @endif
            </div>
